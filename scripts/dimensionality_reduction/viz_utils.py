...
def _plot_cumulative_explained_var(explained_variance_ratio, kl, thres, ax):
    ...
    ax.axhline(thres, 0.01, 0.99, linestyle='--', linewidth=1, color='grey',
               label=f'{int(thres * 100)}% variância explicada')
    ax.set_yticks(np.append(ax.get_yticks()[1:-1], [thres]))

    ax.axvline(kl.knee, linestyle='--', linewidth=1, color='#E73F74', label=f'Número ideal de componentes')
    ax.axvline((explained_variance_ratio.cumsum() < thres).sum()+1, linestyle='--', linewidth=1, color='#11A579',
               label=f'Número mínimo de componentes para {int(thres * 100)}% variância explicada')
    ...
    ax.set_ylabel('Variância explicada (proporção acumulada)', fontsize=13, labelpad=15)
...
def _plot_explained_var_ratio(explained_variance_ratio, kl, ax):
    ...
    ax.axhline(avg_explained_var, 0.01, 0.99, linestyle='--', linewidth=1, color='grey',
            label='Média de variância explicada (%)')
    ...
    ax.axvline((explained_variance_ratio > avg_explained_var).sum(), linestyle='--', linewidth=1,
            color='#11A579', label=f'Número de componentes acima da variância explicada média')
    ax.set_xticks(
        np.append(ax.get_xticks()[1:-1], [kl.knee, (explained_variance_ratio > avg_explained_var).sum()]))
    ax.set_xlim(-n_components * 0.02, n_components * 1.02)

    ax.set_ylabel('Variância explicada (proporção)', fontsize=13, labelpad=8)
    ax.legend(fontsize=12, labelspacing=0.5)

def _plot_normalized_explained_var(explained_variance_ratio, kl, ax):
    _plot_optimal_normalized_elbow(explained_variance_ratio, kl, ax, optimal_label='Número ideal de componentes',
                                xlabel='Número de componentes', ylabel='Curva de variância explicada normalizada')

def _plot_optimal_normalized_elbow(values, kl, ax, optimal_label='', xlabel='', ylabel=''):
    n_components = len(values)

    ax.plot([np.nan] + list(kl.y_normalized), color='#332288', label='')
    ax.plot([np.nan] + list(kl.y_difference), color='#008695', label='Curva de diferença')
...
def plot_num_main_contributors(df, df_trans, thres=0.5, n_contributors=5, dim_idx=None, output_path=None,
    ...
    for pc in mc['component'].unique():
        ax = _get_axis(i, axs, ncols, nrows)
        ...
        if i // ncols == nrows - 1:
            ax.set_xlabel('Coeficiente de correlação', fontsize=12)
...
def _get_axis(i, axs, ncols, nrows):
    ax = None
...
def plot_cumulative_explained_var_comparison(explained_variance_ratio1, explained_variance_ratio2, name1=None,
    ...
    ax.set_ylabel('Variância explicada (proporção acumulada)', fontsize=13, labelpad=15)
    ax.legend(fontsize=12, labelspacing=0.5)
    ax.set_xlabel('Número de componentes', fontsize=13, labelpad=15)
    ax.legend(fontsize=12, labelspacing=0.5)
...
def plot_compare_pca_based_components(components_pca, components_other, original_vars, other_name='Sparse PCA', n_pc=1,
    ...
    plt.legend(fontsize=12, title='Método', title_fontsize=13, labelspacing=0.5)
    plt.xlim(-1, len(original_vars))
    plt.title(f'Componente principal {str(n_pc).zfill(2)}', fontsize=14)
    plt.tight_layout()
    _savefig(output_path=output_path, savefig_kws=savefig_kws)
