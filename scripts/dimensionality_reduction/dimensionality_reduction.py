...
class DimensionalityReduction:
    ...
    def plot_components_violin(
        self,
        title: str = 'Distribution of Dimensionality Reduction Components',
        palette: str = 'viridis',
        output_path: str = None,
        savefig_kws: dict = None
    ):
        """
        Generates a single figure with violin plots to compare the distribution
        of each resulting component.

        This method should be called after running the .transform() method.

        Parameters
        ----------
        title : str, default='Distribution of Dimensionality Reduction Components'
            The title for the plot.
        palette : str, default='viridis'
            The color palette to use for the plots.
        output_path : str, default=None
            Path to save figure as image. If None, the plot is displayed directly.
        savefig_kws : dict, default=None
            Keyword arguments to pass to matplotlib.pyplot.savefig().
        """
        # Check if the transform method has been run
        if self.num_trans_ is None and self.cat_trans_ is None:
            raise RuntimeError("No components found. Please run the .transform() method first.")

        # Combine numerical and categorical components into a single DataFrame
        components_to_plot = []
        if self.num_trans_ is not None:
            components_to_plot.append(self.num_trans_)
        if self.cat_trans_ is not None:
            components_to_plot.append(self.cat_trans_)
        
        df_components = pd.concat(components_to_plot, axis=1)
        ...
        xlabel = 'Component'
        ylabel = 'Value'
        df_melted = df_components.melt(var_name=xlabel, value_name=ylabel)

        # Create the plot figure and axes
        fig, ax = plt.subplots(figsize=(12, 7))

        # Generate the violin plot
        sns.violinplot(x=xlabel, y=ylabel, data=df_melted, palette=palette, hue=xlabel, ax=ax, legend=False)
